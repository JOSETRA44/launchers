...
sealed class CommandAction {
    ...
    data class ToggleDate(val show: Boolean) : CommandAction()
    data class SetStatusBarOpaque(val opaque: Boolean) : CommandAction()
    data class SetCursorBlink(val blink: Boolean) : CommandAction()
    data class SetTypingSpeed(val ms: Int) : CommandAction()
